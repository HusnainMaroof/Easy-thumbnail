...
const Navbar = () => {
  const { setShowLoginPopup } = useAuthContext();
  ...
  return (
    <nav
      className={`w-full bg-white border-b-4 border-black sticky top-0 z-50 transition-all duration-300 ${
    ...
    >
      <div className="max-w-360 mx-auto px-4 sm:px-6 lg:px-10">
        <div className="flex justify-between items-center h-16">
          ...
          {/* MIDDLE: NAV ITEMS */}
          <div className="hidden md:flex items-center gap-1 bg-gray-100/80 backdrop-blur-sm border-2 border-black p-1 rounded-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            {navItems.map((item) => (
          ...
          <div className="flex items-center gap-2 lg:gap-4">
            ...
            <button
              ...
              className="md:hidden p-2 border-4 cursor-pointer border-black bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate[2px] hover:shadow-none transition-all active:bg-black duration-150 rounded-lg"
            >
              {isOpen ? (
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 top-22 md:hidden bg-[#FDFDFF] z-40 flex flex-col p-6 border-t-4 border-black"
          >
